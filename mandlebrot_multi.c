...
/*   Updated: 2022/06/21 17:32:29 by pnoutere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	mandle_inner_loop(t_data *values, t_draw *img)
{
	long double	zx;
	long double	zy;
...
static void	*mandlebrot_multi(void *arg)
{
	t_draw		*img;
	t_data		values;
...
void	assign_tab(t_data *img, t_draw *tab, int i)
{
	tab->nb = i;
	tab->zoom = img->zoom;
	tab->fractal_width = img->fractal_width;
	tab->cx = img->cx;
	tab->cy = img->cy;
	tab->count = img->count;
	tab->iters = img->iters;
	tab->tempx = img->tempx;
	tab->height = img->height;
	tab->width = img->width;
	tab->height = img->height;
	tab->addr = img->addr;
	tab->img = img->img;
	tab->top = img->top;
	tab->left = img->left;
	tab->line_length = img->line_length;
	tab->bits_per_pixel = img->bits_per_pixel;
	tab->psychedelic = img->psychedelic;
	tab->point_in_julia[0] = img->point_in_julia[0];
	tab->point_in_julia[1] = img->point_in_julia[1];
}
...
void	mandle_multithread(t_data *img)
{
	...
	while (i < 8)
	{
		...
		pthread_create(&tids[i], NULL, mandlebrot_multi, (void *)&tab[i]);
		i++;
	}
	i = 0;
	while (i < 8)
