...
/*   Updated: 2022/06/20 14:58:26 by pnoutere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	duck_inner_loop(t_data *values, t_draw *img)
{
	long double	zx;
	long double	zy;
	...
	while (values->x <= values->end)
	{
		...
		while ((zx * zx + zy * zy < 4) && (img->count < img->iters))
		{
			...
			zy = 2 * zx * fabsl(zy) + img->cy;
			zx = img->tempx;
			img->count += 1;
		}
		img->continuous_index = img->count + 34 - log(
				(sqrt(zx * zx + zy * zy) / 2.0f) / log(2)) / log(2);
		my_mlx_pixel_put(values, values->x, values->y,
			color(img->continuous_index));
		values->x++;
	}
}

static void	*duck_multi(void *arg)
{
	t_draw		*img;
	t_data		values;
...
void	weird_duck_multithread(t_data *img)
{
	...
	while (i < 8)
	{
		...
		pthread_create(&tids[i], NULL, duck_multi, (void *)&tab[i]);
		i++;
	}
	i = 0;
	while (i < 8)
