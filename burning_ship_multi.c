...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   burning_ship_multi.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: pnoutere <pnoutere@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/16 10:05:05 by pnoutere          #+#    #+#             */
/*   Updated: 2022/06/21 17:00:25 by pnoutere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void	burning_inner_loop(t_data *values, t_draw *img)
{
	long double	zx;
	long double	zy;
	...
	while (values->x <= values->end)
	{
		...
		img->continuous_index = img->count + 1 - log(
				(sqrt(zx * zx + zy * zy) / 1.1f) / log(2)) / log(2);
...
static void	*burning_multi(void *arg)
{
	t_draw		*img;
	t_data		values;
...
void	burning_multithread(t_data *img)
{
	...
	while (i < 8)
	{
		...
		pthread_create(&tids[i], NULL, burning_multi, (void *)&tab[i]);
		i++;
	}
	i = 0;
	while (i < 8)
